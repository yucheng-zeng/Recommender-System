...
# 读取物品相似度
def readItemSimilarity(item,type):
    if type == 'implicit':
        filename = 'ItemCF/ItemSimilarity/implicit/' + str(item) + '.txt'
    elif type == 'cosSim':
        filename = 'ItemCF/ItemSimilarity/explicit/cosSim/' + str(item) + '.txt'
    elif type == 'ecludSim':
        filename = 'ItemCF/ItemSimilarity/explicit/ecludSim/' + str(item) + '.txt'
    else :
        print('no such type of file')
        return None
